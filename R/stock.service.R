...
#' @param symbols if symbol is "market" then a commas separated list of market symbols
...
dividends <- function (symbol, timePeriod = "3m") {
  ...
  setcolorder(data, c("symbol", setdiff(colnames(data), "symbol")))
  data
...
earnings <- function (symbol, period = "quarter", lastN=1) {
  ...
  data <- lapply(data,function(x){ lapply(x, function(y) {ifelse(is.null(y),NA,y)})});
  tibble::as_tibble(rbindlist(data, fill = TRUE)) %>%
    tibble::add_column(symbol = symbol,.before=1) %>%
    tidyr::unnest_legacy();
};
